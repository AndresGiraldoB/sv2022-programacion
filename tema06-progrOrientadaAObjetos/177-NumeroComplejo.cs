/* Los números complejos son una extensión de los números reales, que se 
emplean en muchas ramas avanzadas de las matemáticas (por ejemplo, en distintos 
sectores de la ingeniería) y que recuerdan mucho a un "vector en dos 
dimensiones", porque están formados por dos números, llamados su "parte real" y 
su "parte imaginaria". En un número de la forma a + bi (2-3i, por ejemplo), la 
parte real sería "a" (2) y la parte imaginaria sería "b" (-3).

Crea una clase NumeroComplejo con:

- Un constructor que permita fijar los valores para la parte real y la parte 
  imaginaria.

- Setters y getters para ambos (llamados SetReal, SetImag y así sucesivamente).

- Un método "ToString", que devolvería "(2, -3)"

- Un método "GetMagnitud" para devolver la magnitud del número complejo (raíz 
  cuadrada de a2 + b2)

- Un método "Sumar", para sumar un número complejo (que se pasará como 
  parámetro) al actual (el resultado será: la parte real será la suma de ambas 
  partes reales, y la parte imaginaria será la suma de ambas partes imaginarias)

- Sobrecarga el operador + para poder realizar esta misma operación de forma 
  más natural.
